...
public class DistanceFinder {
    ...
    public DistanceFinder(DistanceFinderListener listener, String origin, ArrayList<String> destinations) {
    ...
    private String createUrl() throws UnsupportedEncodingException {
        ...
        for (int i=0; i < destinations.size(); i++)
        {
            ...
            urlDestination += URLEncoder.encode(destination, "utf-8");
        }
        String mode = "walking";
        return DISTANCE_MATRIX_DIRECTION_URL + "origins=" + urlOrigin + "&destinations=" + urlDestination + "&mode=" + mode + "&units=metric"+"&key=" + GOOGLE_DIRECTION_API_KEY;
    }
    ...
    private void parseJSon(String data) throws JSONException {
        ...
        List<Destination> rdestinations = new ArrayList<>();
        ...
        for (int i = 0; i < jsonDestinations.length(); i++) {
            ...
            destination.distance = new justrun.running.routeplanner.distanceModule.Distance(jsonDistance.getString("text"), jsonDistance.getInt("value"));
            // change in the future for multiple destinations
            destination.location = this.destination;
            rdestinations.add(destination);
        }

        listener.onDistanceFinderSuccess(rdestinations);
    }
